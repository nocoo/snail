import { HttpError, readBytes } from "../worker/http";
...
export async function assertPublicMediaDns(
  ...
  if (!["video.twimg.com", "pbs.twimg.com"].includes(host))
    throw new HttpError(400, "unsafe_media_address");
  ...
  for (const type of ["A", "AAAA"]) {
    ...
    try {
      ...
      throw new HttpError(422, "media_dns_unavailable");
    }
  ...
  if (!count) throw new HttpError(422, "media_dns_unavailable");
}
