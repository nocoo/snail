...
前端固定 React 19.2.8、TypeScript 7.0.2、Basalt 2.1.7、Vite 8.2.2 和 Tailwind / `@tailwindcss/vite` 4.3.3。Basalt 的 Vite/Tailwind 规范要求先声明扫描路径，再导入共享样式，最后导入 Tailwind。[2] `src/styles.css` 前三行严格为：
...
`vite.config.ts` 使用 `[tailwindcss(), react()]`。不再同时加载 standalone 样式；`.npmrc` 将 `@nocoo` 指向官方 registry，其余依赖和完整性摘要由 `bun.lock` 固定。2026-09-12 两个 Tailwind 4.3.3 官方版本端点均实测 HTTP 200，实际依赖安装成功。[3]

共享库拥有 `--basalt-*`、侧栏宽度、页头和 ContentIsland 的基础样式；应用使用这些 tokens，不重声明颜色、不覆盖默认侧栏宽度和岛内边距。Space Grotesk 应用字体与原始 Snail 品牌资产独立保留。应用的全局元素规则放入 `@layer base`，避免原生按钮重置压过共享组件的 utility 样式。

AppShell / AppMain / AppSkipLink、AppHeader 和 PageHeader 均从各自公共子路径导入；Button、Input、Sidebar、Dialog、Sheet 和 ThemeProvider 使用公共入口。[2] 桌面使用 Sidebar，手机首帧使用 Sheet 抽屉；ThemeProvider 采用默认 `theme` 存储键并支持 system/light/dark。

Biome 2.5.12 对 `@source` 在 `@import` 前仍报 `noInvalidPositionAtImportRule`；只有 `src/styles.css` 针对这一条规则设例外，以保留上游规定的三行顺序。Tailwind 指令解析、其他 lint 规则和实际构建检查继续执行。浏览器回归检查真实计算后的配色、260px 桌面侧栏、56px 页头、手机无侧栏占位、ContentIsland 滚动、主题切换和控件焦点；不能只用文件中出现了 import 字样作为集成通过的证据。
...
[2] Basalt 2.1.7 集成规范：https://github.com/nocoo/basalt/blob/v2.1.7/INTEGRATION.md ；本机同时只读核对已安装公开包的 `dist/styles/tailwind.css`、`tokens.css` 和组件 `.d.ts` / `.js`。

[3] 官方版本元数据：https://registry.npmjs.org/tailwindcss/4.3.3 ；https://registry.npmjs.org/@tailwindcss/vite/4.3.3 。
